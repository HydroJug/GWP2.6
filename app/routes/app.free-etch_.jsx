...
export default function FreeEtchList() {
  ...
  const rowMarkup = discounts.map((d, i) => (
    <IndexTable.Row
      ...
      key={d.id}
      position={i}
      onClick={() => navigate(`/app/free-etch/${encodeURIComponent(d.id)}`)}
    >
      <IndexTable.Cell>
  ...
  return (
    <Page
      ...
      subtitle="Remove etch surcharges when a cart reaches a minimum order value."
      primaryAction={{
    ...
    >
      <TitleBar title="Free Etch Discount" />

      {!functionId && (
        <Box paddingBlockEnd="400">
          <Banner tone="warning">
            <Text variant="bodyMd">
              The Free Etch Discount function is not deployed yet. Run{" "}
      ...
      <Layout>
        <Layout.Section>
          <Card padding="0">
            {discounts.length === 0 ? (
              <EmptyState
                heading="No free etch discounts yet"
                action={{ content: "Create discount", onAction: goToCreate }}
                image="https://cdn.shopify.com/s/files/1/0262/4071/2726/files/emptystate-files.png"
              >
                <p>
                  Create a free etch discount to automatically waive etch
                  surcharges when customers reach a minimum cart value.
                </p>
              </EmptyState>
            ) : (
              <IndexTable
