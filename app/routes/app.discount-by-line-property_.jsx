...
export const loader = async ({ request }) => {
  ...
  const allFns = (fnData.data?.shopifyFunctions?.nodes ?? []).filter(
...
export default function LinePropertyDiscountList() {
  const { functionId, discounts } = useLoaderData();
  const navigate = useNavigate();
  const goToCreate = useCallback(
    () => navigate("/app/discount-by-line-property/new"),
    [navigate]
  );
  ...
  const rowMarkup = discounts.map((d, i) => (
    <IndexTable.Row
      ...
      position={i}
      onClick={() =>
        navigate(`/app/discount-by-line-property/${encodeURIComponent(d.id)}`)
      }
    >
      <IndexTable.Cell>
  ...
  return (
    <Page
      ...
      title="Discount by Line Item Property"
      subtitle="Discount cart lines that carry a specific line item property."
      primaryAction={{ content: "Create discount", onAction: goToCreate }}
    >
      <TitleBar title="Discount by Line Item Property" />
        ...
        <Layout.Section>
          <Card padding="0">
            {discounts.length === 0 ? (
              ...
              >
                <p>
                  Create a discount that automatically applies to cart lines carrying a
                  specific line item property.
                </p>
              </EmptyState>
            ) : (
              <IndexTable
