...
export const loader = async ({ request }) => {
  ...
  const allFns = (fnData.data?.shopifyFunctions?.nodes ?? [])
    .filter((f) => (f.title === "Buy X, Buy Y, Get Z Free" || f.title === "Buy X Buy Y Get Z") && f.apiType === "discount");
...
export default function BuyXYGetZList() {
  ...
  const rowMarkup = discounts.map((d, i) => (
    <IndexTable.Row
      ...
      key={d.id}
      position={i}
      onClick={() => navigate(`/app/buy-xy-get-z/${encodeURIComponent(d.id)}`)}
    >
      <IndexTable.Cell>
  ...
  return (
    <Page
      ...
      subtitle="Require two products/collections in cart to unlock a free gift."
      primaryAction={{
        content: "Create discount",
        onAction: goToCreate,
      }}
    >
      <TitleBar title="Buy X, Buy Y, Get Z Free" />

      {!functionId && (
        <Box paddingBlockEnd="400">
          <Banner tone="warning">
            <Text variant="bodyMd">
              The Buy X Buy Y Get Z function is not deployed yet. Run{" "}
        ...
        <Layout.Section>
          <Card padding="0">
            {discounts.length === 0 ? (
              ...
              >
                <p>
                  Create a Buy X, Buy Y, Get Z Free discount to reward customers
                  who buy specific product combinations.
                </p>
              </EmptyState>
            ) : (
              <IndexTable
