...
export default function BuyMoreSaveMoreList() {
  const { functionId, discounts } = useLoaderData();
  const navigate = useNavigate();
  const goToCreate = useCallback(() => navigate("/app/buy-more-save-more/new"), [navigate]);
  ...
  const rowMarkup = discounts.map((d, i) => (
    <IndexTable.Row
      ...
      key={d.id}
      position={i}
      onClick={() => navigate(`/app/buy-more-save-more/${encodeURIComponent(d.id)}`)}
    >
      <IndexTable.Cell>
  ...
  return (
    <Page
      ...
      subtitle="Tiered quantity price breaks — the more customers buy, the more they save."
      primaryAction={{
        content: "Create discount",
        onAction: goToCreate,
      }}
    >
      <TitleBar title="Buy More, Save More" />

      {!functionId && (
        <Box paddingBlockEnd="400">
          <Banner tone="warning">
            <Text variant="bodyMd">
              The Buy More Save More function is not deployed yet. Run{" "}
      ...
      <Layout>
        <Layout.Section>
          <Card padding="0">
            {discounts.length === 0 ? (
              <EmptyState
                heading="No tiered discounts yet"
                action={{ content: "Create discount", onAction: goToCreate }}
                image="https://cdn.shopify.com/s/files/1/0262/4071/2726/files/emptystate-files.png"
              >
                <p>Create a tiered discount where customers get a better price the more they buy.</p>
              </EmptyState>
            ) : (
              <IndexTable
