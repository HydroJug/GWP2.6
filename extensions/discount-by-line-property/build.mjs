...
// WIT file
const witDir = join(os.tmpdir(), 'discount-by-line-property-wit');
...
function findCliBase() {
  try {
    ...
    // npm global install: realPath ends in .../node_modules/@shopify/cli/bin/run.js
    // Homebrew install: realPath ends in .../libexec/bin/shopify (binary)
    const directBin = dirname(realPath);
    // Check if javy is in the direct bin directory (npm install case)
    if (readdirSync(directBin).some(f => f.startsWith('javy'))) return directBin;
    // Homebrew: navigate up to libexec then down to the CLI bin
