...
export const loader = async ({ request }) => {
  ...
  const allFns = (fnData.data?.shopifyFunctions?.nodes ?? []).filter(
    (f) => f.title === "Tapcart Exclusive Discount Function" && f.apiType === "discount"
  );
...
export default function TapcartList() {
  const { functionId, discounts } = useLoaderData();
  const navigate = useNavigate();
  const goToCreate = useCallback(() => navigate("/app/tapcart/new"), [navigate]);
  ...
  const rowMarkup = discounts.map((d, i) => (
    <IndexTable.Row
      ...
      key={d.id}
      position={i}
      onClick={() => navigate(`/app/tapcart/${encodeURIComponent(d.id)}`)}
    >
      <IndexTable.Cell>
  ...
  return (
    <Page
      ...
      subtitle="App-exclusive discounts for customers shopping through Tapcart."
      primaryAction={{
      ...
      <Layout>
        <Layout.Section>
          <Card padding="0">
            {discounts.length === 0 ? (
              <EmptyState
                heading="No Tapcart discounts yet"
                action={{ content: "Create discount", onAction: goToCreate }}
                image="https://cdn.shopify.com/s/files/1/0262/4071/2726/files/emptystate-files.png"
              >
                <p>
                  Create exclusive discounts that only apply to customers
                  shopping through the Tapcart mobile app.
                </p>
              </EmptyState>
            ) : (
              <IndexTable
